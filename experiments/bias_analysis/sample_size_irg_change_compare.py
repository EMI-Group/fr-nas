...
def run(train_archs, test_archs, draw_archs, weight_factor, name):
    ...
    result = numpy.sum(numpy.abs(stats[0]-stats[1]))
    performance = stats[2]["kendalltau"][0]
    print(result)
...
def main(learning_rate, sampled_archs, sample_size, sample_size_2, sample_size_3, test_size):
    ...
    result_50 = run(train_archs, test_archs, draw_archs, 0, "IRG_feat_50")
    # run(train_archs_2, test_archs, draw_archs, 0, "IRG_feat_200_0")
    result_400 = run(train_archs_3, test_archs, draw_archs, 0, "IRG_feat_400")


    return {"IRG_feat_50":result_50, "IRG_feat_400":result_400}
...
if __name__ == '__main__':
    ...
    with open("result_sample_size_irg_change_compare.json", "w") as f:
        json.dump(trials_result_dict, f)
